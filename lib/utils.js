...
function isOwner(sender) {
    ...
    return numberAllowed.includes(num);
}
...
function isAllowed(senderNumber, fromMe) {
  ...
  // 1. Jika Owner atau Bot Sendiri -> SELALU BOLEH
  if (fromMe || numberAllowed.includes(num)) return true;

  // 2. Cek Mode di Database
  try {
    const dbPath = path.join(process.cwd(), "DATABASE", "settings.json");
    if (fs.existsSync(dbPath)) {
        const settings = JSON.parse(fs.readFileSync(dbPath));
        // Jika mode PUBLIC -> BOLEH
        if (settings.mode === 'public') return true;
    }
  } catch (e) {}

  // 3. Jika Mode SELF (default) -> TOLAK
